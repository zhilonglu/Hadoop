...
*/
public class sta_appointment_success_rate_semiannually {
	public static class TblsWritable implements Writable, DBWritable 
	{  
		int sta_appointment_success_id;//标识  
		double sta_year; 
		double sta_quarter;
		...
		double appointment_success_rate;
		int crest_segment_id;//峰段标识
		String crest_segment_name;//峰段名称
		public TblsWritable() 
		{  
		}  
		public TblsWritable(int sta_appointment_success_id,double sta_year,double sta_quarter,String date_type,int belong_enterprise_id,String belong_enterprise
				,String regionalism_code,String reionalism_name,String belong_province,String belong_city,String belong_country,
				double appointment_success_rate,int crest_segment_id,String crest_segment_name) 
		{  
			this.sta_appointment_success_id = sta_appointment_success_id;//标识  
			this.sta_year = sta_year;
			this.sta_quarter = sta_quarter; 
			...
			this.appointment_success_rate = appointment_success_rate;
		...
		{
			statement.setInt(1, this.sta_appointment_success_id);
			statement.setDouble(2, this.sta_year);
			statement.setDouble(3, this.sta_quarter);
			...
			statement.setDouble(12, this.appointment_success_rate);
			statement.setInt(13, this.crest_segment_id);
			statement.setString(14, this.crest_segment_name);
		}  
		@Override  
		public void readFields(ResultSet resultSet) throws SQLException 
		{  
			this.sta_appointment_success_id = resultSet.getInt(1);//标识  
			this.sta_year = resultSet.getDouble(2); //统计运距统计日期
			this.sta_quarter = resultSet.getDouble(3);
			this.date_type = resultSet.getString(4);//日期类型
			...
			this.appointment_success_rate = resultSet.getDouble(12);
			this.crest_segment_id = resultSet.getInt(13);
			this.crest_segment_name = resultSet.getString(14); 
		}  
		@Override  
		public void write(DataOutput out) throws IOException 
		{  
			out.writeInt(this.sta_appointment_success_id);
			out.writeDouble(this.sta_year);
			out.writeDouble(this.sta_quarter);
			...
			out.writeDouble(this.appointment_success_rate);
		...
		{  
			this.sta_appointment_success_id = in.readInt();//标识  
			this.sta_year = in.readDouble(); //统计运距统计日期
			this.sta_quarter = in.readDouble();
			...
			this.appointment_success_rate = in.readDouble();
			this.crest_segment_id = in.readInt();
			this.crest_segment_name = in.readUTF(); 
		}  
		public String toString() 
		{  
			return new String(this.sta_appointment_success_id +" "+ this.sta_year +" "+this.sta_quarter+" "+this.date_type+" "+
			...
			this.belong_country+" "+this.appointment_success_rate+" "+this.crest_segment_id +" "+this.crest_segment_name);  
		}  
	} 
...
//					context.write(new TblsWritable(keyData[0],keyData[1],string.getKey().toString(),Integer.valueOf(string.getValue().toString())), null);
	...
	public static void main(String args[]) throws IOException, InterruptedException, ClassNotFoundException
	{
		...
		Job job = new Job(conf,"sta_appointment_success_rate_semiannually");  
		job.setJarByClass(sta_appointment_success_rate_semiannually.class);  
