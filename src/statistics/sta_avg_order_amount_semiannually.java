...
*/
public class sta_avg_order_amount_semiannually {
	...
	{  
		...
		double order_complete_total;//总载客距离
		double order_complete_amount;//总订单数
		double order_complete_avg_amount;//平均运距
		int crest_segment_id;//峰段标识
		String crest_segment_name;//峰段名称
		public TblsWritable() 
		{  
		}  
		public TblsWritable(int id,double sta_year,String sta_haflyear,String date_type,int belong_enterprise_id,String belong_enterprise
				,String regionalism_code,String reionalism_name,String belong_province,String belong_city,String belong_country,
				double order_complete_total,double order_complete_amount,double order_complete_avg_amount,int crest_segment_id,String crest_segment_name) 
		{  
			this.id = id;//标识  
			this.sta_year = sta_year;
			this.sta_haflyear = sta_haflyear; //统计运距统计日期
			...
			this.order_complete_amount = order_complete_amount;
			this.order_complete_avg_amount = order_complete_avg_amount;
		...
		public void write(PreparedStatement statement) throws SQLException 
		{
			...
			statement.setString(3, this.sta_haflyear);
			...
			statement.setDouble(12, this.order_complete_total);
			statement.setDouble(13, this.order_complete_amount);
			statement.setDouble(14, this.order_complete_avg_amount);
		...
		{  
			...
			this.sta_haflyear = resultSet.getString(3);
			...
			this.order_complete_total = resultSet.getDouble(12);
			this.order_complete_amount = resultSet.getDouble(13);
			this.order_complete_avg_amount = resultSet.getDouble(14);
		...
		{  
			...
			out.writeUTF(this.sta_haflyear);
			...
			out.writeDouble(this.order_complete_amount);
			out.writeDouble(this.order_complete_avg_amount);
		...
		{  
			...
			this.order_complete_amount = in.readDouble();
			this.order_complete_avg_amount = in.readDouble();
			this.crest_segment_id = in.readInt();
			this.crest_segment_name = in.readUTF(); 
		}  
		public String toString() 
		{  
			return new String(this.id +" "+ this.sta_year +" "+this.sta_haflyear+" "+this.date_type+" "+
			...
			this.belong_country+" "+this.order_complete_total+" "+
			this.order_complete_amount +" "+this.order_complete_avg_amount +" "+this.crest_segment_id +" "+this.crest_segment_name);  
		}  
	} 
...
//					context.write(new TblsWritable(keyData[0],keyData[1],string.getKey().toString(),Integer.valueOf(string.getValue().toString())), null);
	...
	public static void main(String args[]) throws IOException, InterruptedException, ClassNotFoundException
	{
		...
		Job job = new Job(conf,"sta_avg_order_amount_semiannually");  
		job.setJarByClass(sta_avg_order_amount_semiannually.class);  
